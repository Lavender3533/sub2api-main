...
func TestNormalizeOpenAIServiceTierInReqBody(t *testing.T) {
	reqBody := map[string]any{"service_tier": "fast"}

	normalized, changed := normalizeOpenAIServiceTierInReqBody(reqBody)
	require.True(t, changed)
	require.Equal(t, "priority", normalized)
	require.Equal(t, "priority", reqBody["service_tier"])

	normalized, changed = normalizeOpenAIServiceTierInReqBody(map[string]any{"service_tier": "priority"})
	require.False(t, changed)
	require.Equal(t, "", normalized)
}

func TestNormalizeOpenAIServiceTierInRawBody(t *testing.T) {
	normalized, changed, err := normalizeOpenAIServiceTierInRawBody([]byte(`{"service_tier":"fast","model":"gpt-5.4"}`))
	require.NoError(t, err)
	require.True(t, changed)
	require.JSONEq(t, `{"service_tier":"priority","model":"gpt-5.4"}`, string(normalized))

	normalized, changed, err = normalizeOpenAIServiceTierInRawBody([]byte(`{"service_tier":"priority","model":"gpt-5.4"}`))
	require.NoError(t, err)
	require.False(t, changed)
	require.JSONEq(t, `{"service_tier":"priority","model":"gpt-5.4"}`, string(normalized))
}

...
func (s *OpenAIGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*OpenAIForwardResult, error) {
	...
	if normalizedServiceTier, changed := normalizeOpenAIServiceTierInReqBody(reqBody); changed {
		bodyModified = true
		markPatchSet("service_tier", normalizedServiceTier)
	}
...
func (s *OpenAIGatewayService) forwardOpenAIPassthrough(
	...
	normalizedBody, normalizedServiceTier, err := normalizeOpenAIServiceTierInRawBody(body)
	if err != nil {
		return nil, err
	}
...
// 3) service_tier=fast 兼容为 OpenAI API 认可的 service_tier=priority
func normalizeOpenAIPassthroughOAuthBody(body []byte, compact bool) ([]byte, bool, error) {
	...
	next, tierChanged, err := normalizeOpenAIServiceTierInRawBody(normalized)
	if err != nil {
...
func normalizeOpenAIServiceTierValue(raw string) (string, bool) {
...
func normalizeOpenAIServiceTierInReqBody(reqBody map[string]any) (string, bool) {
	if reqBody == nil {
		return "", false
	}
	raw, ok := reqBody["service_tier"].(string)
	...
	normalized, ok := normalizeOpenAIServiceTierValue(raw)
...
func normalizeOpenAIServiceTierInRawBody(body []byte) ([]byte, bool, error) {
	if len(body) == 0 {
		return body, false, nil
	}
	...
	normalized, ok := normalizeOpenAIServiceTierValue(value.String())
	if !ok || value.String() == normalized {
		return body, false, nil
	}
	next, err := sjson.SetBytes(body, "service_tier", normalized)
	if err != nil {
		return body, false, fmt.Errorf("normalize service_tier: %w", err)
	}

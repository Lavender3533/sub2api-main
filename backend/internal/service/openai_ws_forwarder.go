...
func (s *OpenAIGatewayService) ProxyResponsesWebSocketFromClient(
	...
	parseClientPayload := func(raw []byte) (openAIWSClientPayload, error) {
		...
		next, tierChanged, tierErr := normalizeOpenAIServiceTierInRawBody(normalized)
		if tierErr != nil {
			return openAIWSClientPayload{}, NewOpenAIWSClientCloseError(coderws.StatusPolicyViolation, "invalid websocket request payload", tierErr)
		}
		if tierChanged {
			normalized = next
		}
